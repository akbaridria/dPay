...
import { depositAgainETH } from '../../hooks/useStream'
import { depositAgainETHCross } from '../../hooks/useStreamCross'

// interfaces
import { FormStreamMore } from '../../interfaces/globalInterfaces'
...
const props = defineProps<FormStreamMore>()
...
const deposit = async () => {
  ...
  try {
    ...
    if(useWallet.chainId === 5) {
      txHash = await depositAgainETH(chain.contractAddress, props.id, props.totalAmount)
    } else {
      txHash = await depositAgainETHCross(chain.contractAddress, props.id, props.totalAmount, ethers.parseEther(crossFee.value)) 
    }
